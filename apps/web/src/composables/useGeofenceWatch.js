 ...
 */
export function useGeofenceWatch(stationLat, stationLng, geofenceM) {
    ...
    function start() {
        if (!navigator.geolocation || !store.myEntry)
            return;
        if (watchId !== null)
            return; // already watching
        watchId = navigator.geolocation.watchPosition(async (pos) => {
            // Skip if station coordinates not loaded yet
            if (stationLat.value === null || stationLng.value === null)
                return;
            const dist = distanceMeters(pos.coords.latitude, pos.coords.longitude, stationLat.value, stationLng.value);
    ...
    watch([() => store.myEntry, stationLat, stationLng], ([entry, lat, lng]) => {
...
//# sourceMappingURL=useGeofenceWatch.js.map