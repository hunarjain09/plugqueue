...
export const useStationStore = defineStore('station', () => {
    ...
    const myEntry = ref(JSON.parse(localStorage.getItem('pq_my_entry') ?? 'null'));
    ...
    async function validateMyEntry() {
        ...
        try {
            ...
            const found = stationData.queue?.some((q) => q.id === myEntry.value.entry_id);
            if (!found) {
    ...
    async function joinQueue(stationId, plate, spotId, lat, lng, turnstileToken) {
